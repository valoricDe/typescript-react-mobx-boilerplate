...
class QuestionListClass extends Component<Props.IQuestionListProps, void> {
	...
	public render(): JSX.Element {
		...
		const questions = store.searchQuestions || store.allQuestions;
		...
		let items = questionEdges.map(
			(edge, idx) => <QuestionBox store={edge.node} key={idx} />
		);
	...
	fragments: {
		...
		store: (vars) => {
    		...
    		fragment on Query {
				...
				allQuestions(first: 20, orderBy: PRIMARY_KEY_DESC) {
					totalCount
					edges {
						node {
							${QuestionBox.getFragment('store')}
...
export const QuestionListByTag = Relay.createContainer(QuestionListClass, {
	initialVariables: {
		query: null
	},
	prepareVariables: vars => {
